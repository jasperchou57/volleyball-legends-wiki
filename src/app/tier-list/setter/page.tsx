...
export default function SetterTierListPage() {
  const picks = [...featuredStyles]
    .filter((style) => style.bestRoles.includes("Setter"))
  ...
  return (
    <div className="container mx-auto max-w-5xl px-4 py-10">
      ...
      <section className="mt-6 rounded-[2rem] border border-border bg-surface/80 p-6 md:p-8">
        <h1 className="text-4xl font-heading font-black text-white md:text-5xl">Volleyball Legends Setter Tier List</h1>
      </section>
        ...
        {picks.map((style) => (
          <Link key={style.slug} href={`/styles/${style.slug}`} className="rounded-[2rem] border border-border bg-surface/80 p-6 transition hover:border-white/20">
            <div className="flex items-center justify-between gap-3">
              ...
              <span className="rounded-full border border-accent-teal/20 bg-accent-teal/10 px-3 py-1 text-xs font-semibold text-accent-teal">{style.communityTier}</span>
            </div>
